    ...
    private void ContactOtherCube(Collision collision)
    {
        ...
        if (otherCube != null && cube.cubeID > otherCube.cubeID)
        {
            if (cube.CubeNumber == otherCube.CubeNumber)
            {
                ...
                Cube newCube = CubeSpawner.instance.Spawn(cube.CubeNumber * 2, spawnPoint + new Vector3(0, 0.2f, 0));  //yeni oluþan küpün pozisyonu
                ...
                Collider[] surroundedCubes = Physics.OverlapSphere(spawnPoint, 2f); //çarptýðý nesnelerin vereceði tepki
                ...
                foreach (Collider coll in surroundedCubes)
                {
                   if (coll.attachedRigidbody != null)
                       coll.attachedRigidbody.AddExplosionForce(explosionForce, spawnPoint, explosionRadius);
                }

                
                Particle.instance.PlayExplosion(spawnPoint, cube.CubeColor);
